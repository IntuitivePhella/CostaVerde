...
drop trigger if exists update_boat_ratings_trigger on public.reviews;
drop trigger if exists update_reviews_updated_at on public.reviews;

-- Alterar a tabela reviews existente
alter table public.reviews
  ...
  add column if not exists accuracy_rating integer check (accuracy_rating >= 1 and accuracy_rating <= 5),
...
-- Alterar tipo da coluna rating separadamente
alter table public.reviews
alter column rating type decimal(2,1) using rating::decimal(2,1);

-- Adicionar constraint de range após a conversão
alter table public.reviews
add constraint rating_range check (rating >= 0 and rating <= 5);

-- Renomear coluna reviewer_id para user_id para manter consistência
alter table public.reviews
rename column reviewer_id to user_id;
...
-- Recriar os triggers após todas as alterações
